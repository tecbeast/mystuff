...
public class Planet implements IJsonSerializable {
	...
	private void battle(Game game) {
		...
		if (as > 0) {
			LOG.info("Battle at " + fPlanetNr + ":  AS " + as + " DS " + ds);
		}
	  	...
	  	if (as > 0) {
	  		...
	  		report.add("Battle at " + fPlanetName + " (" + fPlanetNr + ") Owner: " + owner.getPlayerName() + " (" + owner.getPlayerNr() + ")");
	...
	private void reportCargo(Game game) {
		...
		Message orbitReport = new Message(Topic.CARGO);
  		...
  		otherReport.add(total + " cargoships approaching " + fPlanetName + " (" + fPlanetNr + ")");
	...
	private int revolt(Game game) {
		...
		if (fPlanetaryDefenseUnits == 0) {
			...
			report.add("Revolution on " + fPlanetName + " (" + fPlanetNr + ")");
	...
	public Message createPlanetReport(Game game, Message message, int level) {
		...
		StringBuilder line = new StringBuilder();
		line.append("Planet: ").append(fPlanetName);
		line.append(" (").append(fPlanetNr).append(")");
